---
title: 🔧 Installer un mod sur Minecraft
description: "Découvrez comment installer des mods Minecraft avec Forge, NeoForge ou Fabric et éviter les plantages grâce au wiki de VikingCraft."
---

# 🔧 Installer un mod sur Minecraft

Les mods permettent d'ajouter de nouvelles fonctionnalités à Minecraft : nouveaux blocs, créatures, dimensions, interfaces, mécaniques de jeu et bien plus encore.

Ce guide vous explique comment installer un mod sur Minecraft Java Edition en quelques minutes.

## 📋 Prérequis

Avant de commencer, assurez-vous d'avoir :

- Minecraft Java Edition installé
- Java installé sur votre ordinateur
- Un mod compatible avec votre version de Minecraft
- Un chargeur de mods comme Forge, NeoForge ou Fabric

## 📥 Télécharger un mod

La première étape consiste à télécharger le mod que vous souhaitez installer.

Avant le téléchargement, vérifiez toujours :

- La version de Minecraft compatible
- Le chargeur de mods requis
- Les éventuelles dépendances nécessaires

Par exemple, un mod prévu pour Minecraft 1.21 avec Fabric ne fonctionnera généralement pas avec Forge.

## ⚙️ Installer un chargeur de mods

La plupart des mods nécessitent un chargeur spécifique.

### Forge

Forge est le chargeur de mods le plus populaire.

- 1. Téléchargez Forge pour votre version de Minecraft.
- 2. Lancez l'installateur.
- 3. Sélectionnez **Installer le client**.
- 4. Attendez la fin de l'installation.

### Fabric

Fabric est souvent utilisé pour les mods légers et les optimisations.

- 1. Téléchargez l'installateur Fabric.
- 2. Lancez-le.
- 3. Choisissez votre version de Minecraft.
- 4. Cliquez sur **Installer**.

### NeoForge

NeoForge est une alternative moderne à Forge utilisée par certains mods récents.

1. Téléchargez l'installateur NeoForge.
2. Exécutez-le.
3. Installez le profil correspondant à votre version.

## 📂 Accéder au dossier Minecraft

Une fois le chargeur installé, vous devez accéder au dossier Minecraft.

### Sous Windows

1. Appuyez sur `Windows + R`
2. Entrez :

```text
%appdata%\.minecraft
```

3. Appuyez sur Entrée.

### Sous macOS

Ouvrez le Finder puis accédez au dossier :

```text
~/Library/Application Support/minecraft
```

### Sous Linux

Le dossier Minecraft se trouve généralement ici :

```text
~/.minecraft
```

## 📦 Installer le mod

Dans le dossier Minecraft :

1. Ouvrez le dossier `mods`
2. S'il n'existe pas, créez-le
3. Déplacez le fichier `.jar` du mod dans ce dossier

Exemple :

```text
.minecraft
└── mods
    └── mon_mod.jar
```

Vous pouvez installer plusieurs mods dans ce dossier tant qu'ils sont compatibles entre eux.

## 🚀 Lancer Minecraft

Une fois le mod installé :

- 1. Ouvrez le launcher Minecraft
- 2. Sélectionnez le profil Forge, NeoForge ou Fabric
- 3. Lancez le jeu

Si tout est correctement installé, le mod sera chargé automatiquement au démarrage.

## 🛠️ Résoudre les problèmes courants

### Le jeu plante au lancement

Vérifiez que :

- Le mod correspond à votre version de Minecraft
- Vous utilisez le bon chargeur de mods
- Toutes les dépendances sont installées
- Aucun autre mod n'est incompatible

### Le mod n'apparaît pas

Assurez-vous que :

- Le fichier `.jar` est dans le dossier `mods`
- Vous avez sélectionné le bon profil dans le launcher
- Le mod est compatible avec votre version de Minecraft

### Message d'erreur lié à une dépendance

Certains mods nécessitent d'autres mods pour fonctionner.

Lisez attentivement la page de téléchargement afin de vérifier les dépendances requises.

## 💡 Conseils

- Téléchargez vos mods uniquement depuis des sites reconnus.
- Vérifiez toujours la compatibilité des versions.
- Sauvegardez vos mondes avant d'installer de nouveaux mods.
- Installez les mods un par un afin d'identifier facilement les éventuels problèmes.
- Évitez d'installer un grand nombre de mods sans tester votre jeu entre chaque ajout.

## 🎮 Tester vos mods sur un serveur Minecraft
Une fois votre mod installé, vous pouvez également le tester en multijoueur sur un serveur compatible.
Si vous recherchez un serveur Minecraft francophone proposant une économie avancée, des métiers, des compétences, des donjons, des quêtes et de nombreuses fonctionnalités inédites, vous pouvez découvrir **VikingCraft**.

**Adresse du serveur :** `go.vikingcraft.fr`

Avant de rejoindre un serveur avec des mods, vérifiez toujours que celui-ci est compatible avec votre version de Minecraft et les mods installés sur votre client.


## ✅ Conclusion

Vous savez désormais comment installer un mod sur Minecraft Java Edition.

En utilisant Forge, NeoForge ou Fabric et en respectant les versions compatibles, vous pourrez profiter facilement de milliers de mods pour personnaliser votre expérience de jeu.


